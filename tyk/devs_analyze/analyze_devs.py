...
def parseDB():
    sql_parse = """SELECT ID, user_email
                    ...
                    """
    cur_sel.execute(sql_parse)
    indexs = []
    new_domains = []
    
    for index, mail in cur_sel.fetchall():
        try:
    ...
    for user_id, domain in zip(indexs, new_domains):
        sql_inject = """INSERT INTO wp_usermeta (user_id, meta_key) 
                        ...
                        FROM wp_users users
                        WHERE users.ID = %s"""
        sql_update = "UPDATE wp_usermeta SET meta_value = %s WHERE user_id = %s AND meta_key = 'priority'"
        ...
        cur_inj.execute(sql_inject, (user_id, ))
        cur_up.execute(sql_update, (domain_type, user_id,))
