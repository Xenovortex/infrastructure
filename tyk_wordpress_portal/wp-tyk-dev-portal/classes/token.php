...
class Tyk_Token
{
    ...
    public function revoke() {
        ...
        try {
            ...
            //     if (Tyk_Dev_Portal::is_hybrid()) {
                    ...
                    PC::debug(sprintf('/apis/%s/keys/%s', 
                        $apis[3]->api_definition->api_id,
                        $this->key                       
                    ));
                    PC::debug($this->api);
                    ...
                    if (is_object($response) && isset($response->Status) && $response->Status == 'OK') {
                        PC::debug($response, 'returning');
                        return true;
                    }
                    else {
                        PC::debug($response, 'hmmm');
                        throw new Exception('Received invalid response from Tyk Api');
                    }
    ...
    public function get_usage_quota() {
	    ...
	    //$obj -> request = sprintf('/keys/%s', $this->key);
