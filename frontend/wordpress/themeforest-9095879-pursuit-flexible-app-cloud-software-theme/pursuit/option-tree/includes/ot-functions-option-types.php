 ...
 * @since     2.0
 */
if ( ! function_exists( 'ot_type_sortable' ) ) {
  
  function ot_type_sortable( $args = array() ) {
    ...
    /* format setting outer wrapper */
    echo '<div class="format-setting type-sortable ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
      
      /* description */
      echo $has_desc ? '<div class="description">' . htmlspecialchars_decode( $field_desc ) . '</div>' : '';
      
      /* format setting inner wrapper */
      echo '<ul class="format-setting-inner" id="themo_sortable">';    
		...
		foreach ( (array) $field_choices as $key => $choice ) {
          if ( isset( $choice['value'] ) && isset( $choice['label'] ) ) {
			...
			echo '<li class="ui-state-default">';			  
			  ...
			  echo '<input type="hidden" data-meta-box-name="'.$meta_box_name.'"  name="' . esc_attr( $field_name ) . '[' . esc_attr( $key ) . ']" id="' . esc_attr( $field_id ) . '-' . esc_attr( $key ) . '" value="' . ( isset( $field_value[$key] ) ?  $field_value[$key] : esc_attr( $choice['value'] ) ) . '" class="option-tree-ui-checkbox ' . esc_attr( $field_class ) . '" />';
              echo '<label for="' . esc_attr( $field_id ) . '-' . esc_attr( $key ) . '">' . esc_attr( $choice['label'] ) ; 
			  //echo " | Sort Order Value: " .( isset( $field_value[$key] ) ?  $field_value[$key] : esc_attr( $choice['value'] ) ) . "<br>";
			  echo '</label>';
			  
			  
			  
			  //echo '<input type="hidden" data-meta-box-name="'.$choice['label'].'" name="' . esc_attr( $field_name ) . '[' . ( isset( $field_value[$key] ) ?  $field_value[$key] : esc_attr( $key ) ) . ']" id="' . esc_attr( $field_id ) . '-' . ( isset( $field_value[$key] ) ?  $field_value[$key] : esc_attr( $key ) ) . '" value="' . esc_attr( $choice['value'] ) . '" class="' . esc_attr( $field_class ) . '" />';
              //echo '<label for="' . esc_attr( $field_id ) . '-' . ( isset( $field_value[$key] ) ?  $field_value[$key] : esc_attr( $key ) ) . '">' . esc_attr( $choice['label'] );
			  //echo " | Key: ". esc_attr( $key ) . " | DB Value: ". $field_value[$key] . " | choice value: " .esc_attr( $choice['value'] ) . " | Final Result: " . ( isset( $field_value[$key] ) ?  $field_value[$key] : esc_attr( $choice['value'] ) ) . "<br>";
			  //echo " | choice value: " .esc_attr( $choice['value'] ) . "<br>";
			  echo '</label>';
            echo '</li>';
          }
        }
