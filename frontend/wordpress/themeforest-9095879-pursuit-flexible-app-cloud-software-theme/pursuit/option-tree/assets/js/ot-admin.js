...
;(function($) {
  OT_UI = {
        ...
        $.ajax({
          ...
          complete: function( data ) {
			...
			setTimeout( function() {
				...
				/* Future Update, active html tab and / or add tinymce editor */
				$('#setting_'+name+' .switch-html').each(function () {
					
					var activeEditor = tinyMCE.activeEditor.id;
					var activeEditorFix =  activeEditor+'-html';
					
					var activeTextarea = this.id;
					
				  
					if(activeEditorFix == activeTextarea){
						console.log('pass');
					}else{
						console.log("active Editor "+ activeEditor);
						console.log("active Textarea "+ activeTextarea);
						console.log(activeTextarea +" switch to text mode");
    ...
    parse_condition: function(input_name) {
		
		//console.log('parse_condition called');
		/*
		THEMO NOTE: Passed input_name to filter .each statement to improve performance
		themo_accordion_1[0][themo_accordion_1_show_button]
		themo_accordion_1_themo_accordion_1_show_button_0:is(on)
		*/
		if(typeof input_name === 'undefined'){
			//console.log('undefined');
			var data_condition_name = 'data-condition';
		}else{
			if (input_name.indexOf("[") >= 0){
				//console.log('Found a [');
				var data_condition_name = 'data-condition';
			}else{
				//console.log('Nothing found, continue');
				var data_condition_name = 'data-condition^="'+input_name+'"';
			}
			
		}
		
		$( '.format-settings[id^="setting_"]['+data_condition_name+']' ).each(function() {
    ...
    init_conditions: function() {
		//console.log('init_conditions called');
		/*
		THEMO NOTE: Created click even to replace code that was commented out.
		*/
		/*$('input.option-tree-ui-radio').on("click", function() {           
			console.log('option-tree-ui-radio CLICK');
			OT_UI.parse_condition($( this ).attr("name"));
		});*/
		$(document).on('click', 'input.option-tree-ui-radio', function() {
			//console.log('NEW option-tree-ui-radio CLICK');
			OT_UI.parse_condition($( this ).attr("name"));
		});
		/*
		THEMO NOTE: Commented out to improve performance.
		
		$('.format-settings[id^="setting_"]').on( 'change.conditionals, keyup.conditionals', OT_UI.condition_objects(), function() {
		//$('.format-settings[id^="setting_"]').on( 'change.conditionals', OT_UI.condition_objects(), function() {
			//var start = new Date().getTime();
			OT_UI.parse_condition();
			//var end = new Date().getTime();
			//var time = end - start;
			//console.log('parse_condition() Execution time Triggered by keyup.conditionals: ' + time);
		});*/
      //console.log('init_conditions parse_condition');
	  OT_UI.parse_condition();
    },
...
!function ($) {
  ...
  $(document).on('ready', function () {
	...
	//$(".ot-metabox-wrapper").click(doMetaBox_tabs);
...
!function ($) {
  ...
  $(document).on('ready', function () {
	...
	// Hide Meta Box Options
	//$( ".not-sortable .ot-metabox-wrapper").hide();
	$("div[id*='themo_'][id$='_meta_box'] .ot-metabox-wrapper").hide();
	$("div[id*='themo_'][id$='_meta_box'] .inside:has(.ot-metabox-wrapper)").append( "<p class='edit_meta_box'>Click here to edit.</p>" );
	
	// Add Button to Edit
	//$( ".not-sortable .inside:has(.ot-metabox-wrapper)" ).append( "<p class='edit_meta_box'>Click here to edit.</p>" );
	
	// Reverse hide and helpful text for page layout meta box
	$( "#themo_page_layout_meta_box .ot-metabox-wrapper").show();
	$( "#themo_page_layout_meta_box .edit_meta_box").remove();
	
	
	// Click event for edit button
	$(".edit_meta_box").click( function(){
		...
		// Show Meta Box
		$(this).parent().find('.ot-metabox-wrapper').show();
		
		// Show TinyMCE
		/* Since we are hiding the wrapper we need to remove the tmce plugin for it to come back to life again. */
		$(this).parent().find('.wp-editor-area').each( function() {
			var tinymce_id = $(this).attr('id')
			console.log('Remove TMCE: '+tinymce_id);			
			/*tinyMCE.execCommand('mceFocus', false, tinymce_id);
			tinyMCE.execCommand('mceRemoveEditor',false, tinymce_id);
			tinyMCE.execCommand('mceAddEditor',false, tinymce_id);*/
			/*tinymce.execCommand('mceRemoveEditor', true, tinymce_id);
			var init = tinymce.extend( {}, tinyMCEPreInit.mceInit[ tinymce_id ] );
			try { tinymce.init( init ); } catch(e){}*/
		})
		
		$(this).parent().find('.switch-tmce').each(function () {
			//console.log('Switching to visual mode ');
			switchEditors.switchto(this);
		});
		
		// Remove Edit button
		$(this).parent().find(".edit_meta_box").remove();
				
	}); // END Click Event
	
	
	//$(".ot-metabox-wrapper").click(make_meta_box_tabs);
	
	function make_meta_box_tabs() {
